...
### FR-005: ソースコードとASTツリーの連動表示
**優先度**: 必須  
**説明**: ソースコード上の要素をクリック・選択することで、その部分に対応するASTノードをツリービューでハイライト表示する  
**詳細**:
- ソースコードビューとASTビューの連動
- 選択時にASTツリーの該当ノードをハイライト表示
- 該当ノードが折りたたまれている場合は、そこまでの枝を自動展開
- ツリー側でノードを選択した場合、ソースコードの該当箇所もハイライト（推奨）
...
**検証基準**:
- [ ] ソースコード上でクリック・選択すると対応ASTノードがハイライトされる

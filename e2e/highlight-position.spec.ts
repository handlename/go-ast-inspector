...
test.describe("Highlight Position Accuracy", () => {
	...
	test("should accurately position highlight for nodes at various positions", async ({
		page,
	}) => {
		...
		for (const index of testIndices) {
			...
			const highlightMark = page.locator(
				".code-editor__syntax-pre .code-highlight",
			);
	...
	test("should maintain highlight-cursor alignment with narrow viewport", async ({
		page,
	}) => {
		...
		const funcNodes = page
			.locator(".tree-node__type")
			.filter({ hasText: "FuncDecl" });
		...
		if (funcCount > 1) {
			...
			if (markBox && textareaBox) {
				...
				expect(markBox.x).toBeLessThanOrEqual(
					textareaBox.x + textareaBox.width + 10,
				);
			}
		}
	});

	test("should compare textarea cursor position with highlight position", async ({
		page,
	}) => {
			...
			if (markCount > 0) {
				...
				if (markBox) {
					...
					if (layerBox) {
						...
						const isVisible =
							markBox.y >= layerBox.y - markBox.height &&
							markBox.y <= layerBox.y + layerBox.height;
	...
	test("should measure rendering difference between textarea and pre", async ({
		page,
	}) => {
		...
		if (comparison) {
			...
			const widthDiff = Math.abs(
				comparison.textarea.width - comparison.pre.width,
			);
...
test.describe("Detailed Position Analysis", () => {
	test("should measure line-by-line height accumulation", async ({ page }) => {
		...
		const linePositions = await page.evaluate((code: string) => {
			...
			const syntaxLayer = document.querySelector(
			...
			for (
				let i = 0;
				i < Math.min(textareaLinePositions.length, preLinePositions.length);
				i++
			) {

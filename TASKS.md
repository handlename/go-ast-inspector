...
- [ ] TypeScript設定
  ...
  - 詳細:
    ...
    - strict modeを有効化
  - 完了条件:
    - [ ] tsconfig.jsonが存在し、DESIGN.mdの仕様に従っている
    - [ ] `tsc --noEmit`がエラーなく実行できる
...
- [ ] Vite設定
  ...
  - 詳細:
    ...
    - パスエイリアス（$lib）を設定
  - 完了条件:
    - [ ] vite.config.tsが存在し、DESIGN.mdの仕様に従っている
...
- [ ] Svelte設定
  ...
  - 完了条件:
    - [ ] svelte.config.jsが存在し、DESIGN.mdの仕様に従っている
    - [ ] Svelteファイルが正しくコンパイルされる
...
- [ ] Biome設定
  ...
  - 完了条件:
    - [ ] biome.jsonが存在し、DESIGN.mdの仕様に従っている
    - [ ] `npm run lint`がエラーなく実行できる
...
- [ ] WASM開発環境構築
  ...
  - 完了条件:
    - [ ] src/wasm/build.shが実行可能
    - [ ] `npm run build:wasm`が成功する
    - [ ] public/parser.wasmとpublic/wasm_exec.jsが生成される
...
- [ ] 基本的なASTパーサー実装
  ...
  - 完了条件:
    - [ ] 有効なGo言語コードをASTに変換できる
    - [ ] 構文エラー時に行番号・列番号を含むエラー情報を返す
    - [ ] JavaScriptから呼び出し可能
...
- [ ] CodeEditorホバー機能実装
  - 目的: マウスホバー時に対応するASTノードを特定する
  - 詳細:
    - mousemoveイベントのハンドリング
    - カーソル位置の計算
...
- [ ] ASTTreeView基本実装
  ...
  - 詳細:
    ...
    - 空状態の表示
  - 完了条件:
    - [ ] ASTが存在する場合にツリーが表示される
    - [ ] 空状態が適切に表示される
    - [ ] Expand/Collapse Allボタンが機能する
...
### 4-4. HoverTooltipコンポーネント実装

- [ ] HoverTooltip基本実装
  - 目的: ホバー時にASTノード情報を表示するツールチップを実装する
  - 詳細:
    - src/components/HoverTooltip.svelteを作成
    - selectedNodeStoreの購読
    - ツールチップの表示/非表示制御
    - ノードタイプ、位置情報の表示
  - 完了条件:
    - [ ] ホバー時にツールチップが表示される

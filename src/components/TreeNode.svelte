<script lang="ts">
  ...
  export let defaultExpandLevel: number;

  $: isExpanded = $expandedNodesStore.has(nodeId);
  $: isSelected = $selectedNodeStore?.pos === node.pos && $selectedNodeStore?.end === node.end;
  ...
  $: metadataStr = formatMetadata(node.metadata);

...
<div class="ast-tree-view">
  <div class="ast-tree-view__header">
    ...
    <div class="ast-tree-view__controls">
      <button class="ast-tree-view__button" on:click={expandAll}>
        Expand All
      </button>
      <button class="ast-tree-view__button" on:click={collapseAll}>
        Collapse All
      </button>
    </div>
  </div>
  ...
  <div class="ast-tree-view__content">
    {#if $astStore}
      <TreeNode node={$astStore} level={0} nodeId="0" defaultExpandLevel={DEFAULT_EXPAND_LEVEL} />
    {:else}
      <p class="ast-tree-view__empty">No AST available. Enter valid Go code to see the AST.</p>
    {/if}
  </div>
</div>
